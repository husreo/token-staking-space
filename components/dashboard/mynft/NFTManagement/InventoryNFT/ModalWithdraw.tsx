...
const ModalWithdrawNFT = ({
  ...
  const { data, loading, mutate } = useRequest(
    async () => {
      const req = await fetch("/api/nft/withdraw-nft-solana", {
        method: "POST",
        body: JSON.stringify({
          token_address: nft_id,
  ...
  }, [wallet_address, nft_id, mutate, reset]);

  return (
    <ModalWrapper open={!!nft_id && !!wallet_address} onClose={() => {}}>
      <div className="relative min-h-[200px] w-[560px] rounded-[20px] bg-gray2 p-6 text-white">
        ...
        <div>
          <p className="mb-5 text-center text-2xl font-medium">Withdraw NFT</p>
          <div className="flex items-center gap-2 rounded-[10px] bg-white/[0.03] p-2">
            <div className="relative h-20 w-20 overflow-hidden rounded-[10px]">
              {logo_url && (
                <Image src={logo_url} alt="" fill className="object-cover" />
              )}
            </div>
            <p>{nft_name}</p>
          </div>
