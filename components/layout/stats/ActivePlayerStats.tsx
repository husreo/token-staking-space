...
import TableGameStat from "./TableGameStat";
...
export default function ActivePlayerStats() {
  ...
  const handleGetGamestats = async () => {
    try {
      ...
      const req = await fetch(
        `/api/platform/game-stats/active-players?from_date=${formatFromDate}&to_date=${formatToDate}&is_all=${isAllDate}`,
        {
          method: "GET",
        },
      );
  ...
  return (
    <>
      <PoolChart
        ...
        labelTitle={t("stats.active-player")}
        keyTable="player_stats"
        isLoading={isLoading}
      />
      <TableGameStat data={data} isLoading={isLoading} />
    </>
  );
}
