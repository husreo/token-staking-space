...
const LoaderContainer = ({ children }: { children: React.ReactNode }) => {
  return (
    <div className="fixed left-0 top-0 z-[9999] h-full w-full text-white transition-all ">
      {children}
    </div>
  );
};
const LoaderSpinner = () => {
  return (
    <div
      ...
      className={`absolute left-1/2 top-1/2 flex h-[120px] w-[120px] -translate-x-1/2 -translate-y-1/2 transform items-center justify-center overflow-hidden rounded-full border border-transparent bg-[center_center] bg-no-repeat text-center shadow-[0px_0px_24px_4px_rgba(#2a3d38,0.2)]`}
    >
      <RocketIcon className="w-[32px]" />
      <div
        style={{
          backgroundImage:
